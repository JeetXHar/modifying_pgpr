...
class BatchKGEnvironment(object):
    def __init__(self, dataset_str, max_acts, max_path_len=3, state_history=1, optimize_for=None, alpha=None):
        ...
        self.patterns_extend = list(tuple([w[0] for w in v]) for v in PATH_PATTERN["ML1M_extend"].values())
    ...
    def _has_pattern(self, path):
        pattern = tuple([v[0] for v in path[:4]])
        flag=pattern in self.patterns

        for i in range(4,len(path),2):
            flag=(flag and (tuple([v[0] for v in path[i:i+2]]) in self.patterns_extend))
        #if len(path)>4:
        #    print(flag,path)
        return flag
    ...
    def _get_actions(self, path, done):
        ...
        # Build per-relation buckets
        by_rel = {}
        for r, next_node_id in candidate_acts:
            by_rel.setdefault(r, []).append(next_node_id)
    
        # Total candidates
        N_total = len(candidate_acts)
    
        # Compute per-relation quotas
        quotas = {}
        for r, nodes in by_rel.items():
            frac = float(len(nodes)) / float(N_total)
            k_r = max(1, int(math.floor(frac * self.max_acts)))
            quotas[r] = k_r
    
        # Score and select top-k_r from each bucket
        selected = []
        for r, nodes in by_rel.items():
            scores_r = []
            for e_id in nodes:
                ...
                if next_node_type == USER:
                    ...
                    src_embed = user_embed + self.embeds[main_product][0] + self.embeds[r][0]
                score = np.dot(src_embed, self.embeds[next_node_type][e_id])
                scores_r.append((e_id, score))
    
            # Keep top-k_r by score
            topk = sorted(scores_r, key=lambda x: x[1], reverse=True)[:quotas[r]]
            for e_id, _ in topk:
                selected.append((r, e_id))
    
        # If we still have too many, trim globally by score
        if len(selected) > self.max_acts:
            global_scores = []
            for r, e_id in selected:
                next_node_type = KG_RELATION[self.dataset_name][curr_node_type][r]
                if next_node_type == USER:
                    src_embed = user_embed
                elif next_node_type == main_product:
                    ...
                    src_embed = user_embed + self.embeds[main_product][0] + self.embeds[r][0]
                score = np.dot(src_embed, self.embeds[next_node_type][e_id])
                global_scores.append((r, e_id, score))
    
            # Keep only the top-max_acts globally
            global_top = sorted(global_scores, key=lambda x: x[2], reverse=True)[:self.max_acts]
            selected = [(r, e_id) for (r, e_id, _) in global_top]
    
        # Final action list
        actions.extend(sorted(selected, key=lambda x: (x[0], x[1])))
        return actions
